...
export default function Submissions() {
  ...
  const [randomlySortedSubmissions, setRandomlySortedSubmissions] = useState<Submission[]>([]);

  useEffect(() => {
    setRandomlySortedSubmissions(shuffleArray(submissions));
  }, []);
              ...
              <CardHeader className="justify-between">
                <div className="flex gap-5">
                  ...
                  <div className="flex flex-col gap-1 items-start justify-center">
                    <h4
                      className="font-semibold leading-none text-default-600 mb-2 text-xl ">{(s.name || 'Untitled Submission') + ' by ' + s.authorGithub}</h4>
                  </div>
                  {
                    (s.authorTwitter && (
                      <Link
